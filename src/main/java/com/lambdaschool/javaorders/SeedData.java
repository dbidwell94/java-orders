    ...
    public void run(String[] args) throws Exception {
        ...
        custrepos.save(c25);
        {
            ...
            for (int i = 0; i < 1000; i++) {
                Address a = jFaker.address();
                int grade = new Random().nextInt(4);
                int agentIndex = new Random().nextInt(agents.size());
                Customer newC = new Customer(jFaker.funnyName().name(),
                        a.city(),
                        a.city(),
                        a.country(),
                        Integer.toString(grade),
                        Double.parseDouble(jFaker.commerce().price()),
                        Double.parseDouble(jFaker.commerce().price()),
                        Double.parseDouble(jFaker.commerce().price()),
                        Double.parseDouble(jFaker.commerce().price()),
                        jFaker.phoneNumber().phoneNumber(),
                        agents.get(agentIndex));
                custrepos.save(newC);
                for(int x = 0; x < new Random().nextInt(11) ; x++){
                    Order newOrder = new Order(
                            Double.parseDouble(jFaker.commerce().price()),
                            Double.parseDouble(jFaker.commerce().price()),
                            newC,
                            jFaker.backToTheFuture().character()
                    );
